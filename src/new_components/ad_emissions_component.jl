@defcomp ad_emissions begin
    ...
    CARBON_CONSTRAINT_PENALTY = Variable(index=[time, regions]) # Penalty for exceeding carbon budget
    CBUDGET = Parameter() # Cumulative global carbon budget (GtC)
    ...
    function run_timestep(p, v, d, t)
        ...
        if is_last(t)
            excess_carbon = v.CCA[t] - p.CBUDGET
            if abs(excess_carbon) > 0.1
                for r in d.regions
                    v.CARBON_CONSTRAINT_PENALTY[t,r] = p.CONSTRAINT_PENALTY_STRENGTH * (excess_carbon * 1000.0)^2
                end
            else
                for r in d.regions
                    v.CARBON_CONSTRAINT_PENALTY[t,r] = 0.0
                end
            end
        end
